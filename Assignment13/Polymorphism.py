...
class SUV(Vehicle):
    ...
    def get_info(self):
        return f"Make: {self.make}, Model: {self.model}, Year: {self.year}, Cargo Capacity: {self.cargo_capacity}"
...
if __name__ == "__main__":
    ...
    my_car = Car("Honda", "Accord", 2022, 4)
    my_suv = SUV("Toyota", "Highlander", 2023, 83.7)
    my_truck = Truck("Ford", "F-250", 2024, 10000)

    # Demonstrate polymorphism with 'get_info()' 
    print("Vehicle Information:")
    print(f"Car: {my_car.get_info()}")
    print(f"SUV: {my_suv.get_info()}")
    print(f"Truck: {my_truck.get_info()}")

    # Print class information
    print("\nClass Information:")
    print(f"Car: {type(my_car)}")
    print(f"SUV: {type(my_suv)}")
    print(f"Truck: {type(my_truck)}")
